    ...
    if (req.file) {
      try {
        ...
        return next(
          createError(
            500,
            error.message || "file not uploaded , plese try again"
          )
        );
      }
    }
...
export const forgotPassword = async (req, res, next) => {
  ...
  const message = `You can reset your password by clicking <a href="${encodeURI(
    resetPasswordUrl
  )}" target="_blank">Reset your password</a>. If the above link does not work for some reason, then copy-paste this link in a new tab: ${encodeURI(
    resetPasswordUrl
...
// Add child to parent's childs array
export const addChildToParent = async (req, res, next) => {
  try {
    const { parentEmail, userEmail } = req.body;
    if (!parentEmail || !userEmail) {
      return next(
        createError(400, "Both parentEmail and userEmail are required")
      );
    }

    const parent = await User.findOne({ email: parentEmail });
    if (!parent) {
      return next(createError(404, "Parent user not found"));
    }

    const child = await User.findOne({ email: userEmail });
    if (!child) {
      return next(createError(404, "Child user not found"));
    }
    ...
    const childInfo = {
      ...
      email: child.email,
      avatar: {
        public_id: child.avatar?.public_id,
        secure_url: child.avatar?.secure_url,
      },
    };

    // Prevent duplicate child entries
    const alreadyAdded = parent.childs.some((c) => c.email === child.email);
    if (alreadyAdded) {
      return res
        .status(400)
        .json({ success: false, message: "Child already added to parent" });
    }


    child.parent_email = parentEmail;
    await child.save();
    parent.childs.push(childInfo);
    await parent.save();
